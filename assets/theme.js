...
$(document).ready(function(){
  $('.atc-swatch').on('click',function(){
    var outOfStock = $(this).attr('data-available');
    var variantProdId = $(this).attr('data-product-id');
    var parentImage = $(this).parents('.related-cart').find('.atc-image');
    var parentBtn = $(this).parents('.related-cart').find('.atc-btn'); 
    var parentVariantText = $(this).parents('.related-cart').find('.atc-span');
    $(parentImage).attr('src',$(this).attr('data-main-image'));
    $(parentVariantText).text($(this).attr('alt'));
    if(outOfStock == 0){
      $(parentBtn).addClass('disabled').attr("disabled", "disabled");
    }
    else{
      $(parentBtn).removeClass('disabled').removeAttr("disabled").attr('href','/cart/add?id='+ variantProdId +'&quantity=1');
    }
  })
  $('.sip-btn').on('click',function(e){
      e.preventDefault();
      $(this).parents('.overlay-image').find('img').css('display','none');
  })
  $("#starDesigning").on('click',function(e) {
    e.preventDefault();
    $( ".custom-floatie-iframe" ).toggle( "slow");
  	$('.pr-custom-order .grid').css('height','auto');
    $('html, body').animate({
      scrollTop: jQuery('.custom-floatie-iframe').offset().top - 150
  }, 2000);
  });  
  var str = $('.header-bar__message a').text();
  $('.header-bar__message a').html(str.replace("These Frames", "<u>These Frames</u>"));
  $('.btn-search-prive').on('click',function(e){
      e.preventDefault();
      $('.search-prive').toggleClass('visible');
  }); 
